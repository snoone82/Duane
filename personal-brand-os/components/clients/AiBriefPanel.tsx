...
export function AiBriefPanel({ clientId, planId }: { clientId: string; planId: string }) {
  ...
  function runImport() {
    ...
    startImport(async () => {
      ...
      const { masterContentCreated, platformOutputsCreated, requirementsCreated, requirementsFlagged, requirementsAutoGenerated, warnings } =
        result.data;
      const requirementBits = [
        ...
        requirementsFlagged > 0 ? `${requirementsFlagged} needs confirmation` : null,
      ].filter(Boolean);
      setImportSummary(
        `Imported ${masterContentCreated} Master Content, ${platformOutputsCreated} Platform Output(s)` +
          (requirementBits.length > 0 ? `, and Requirements (${requirementBits.join(", ")}).` : ".")
      );
