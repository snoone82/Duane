...
Live project: **`personal-brand-os`**, ref `lqqwaybkjuqfpmodsmbf`, org ESSENN, region `eu-west-2`. Every migration in [`supabase/migrations/`](supabase/migrations/) is applied and live, not just written — `lib/database.types.ts` is generated from the real schema, not hand-written.
...
- **`portal_user_id`** — one linked login per client, set by an admin from the Overview tab. Portal reads go through explicit SELECT-only RLS policies plus the `portal_meeting_summaries` safe-column view (SECURITY DEFINER with its own `is_portal_client_of()` guard — the advisor flags the pattern, but it's deliberate: it's the only way to narrow *columns*, not just rows, for portal eyes).
...
## Phase Two (built after Duane's go-ahead)

The four items originally deferred are now in:

- **Social Media Strategy (§10)** — a **Social** tab per client: one expandable card per platform (objective, audience, content types, posting frequency, growth/engagement/CTA strategy), add/delete platforms, autosave throughout. Table `social_strategies`, unique per client+platform.
- **Sales Strategy (§12)** — a **Sales** tab: the brief's eleven fields grouped into "What they sell" / "How the brand sells" / "Turning interest into revenue", framed by Visibility → Authority → Trust → Opportunity → Revenue. Strategic-tier RLS — contractors get a 404, same as Vision. Row auto-created per client (`create_client_defaults`).
- **Client portal (§22)** — a read-only surface at `/portal` for accounts with the new `client` role, linked to one client via `clients.portal_user_id` (admin-only "Client portal" section on Overview does the linking). Five sections: Strategy, Priorities, Content, Progress, Meetings. Meeting records are exposed **only** through the `portal_meeting_summaries` view (date, type, summary, wins, next meeting) — never the internal strategic notes — and portal RLS policies grant SELECT only on the §22 list (never consultations, scorecard, commercial tables, sales strategy, or files). `has_client_access()` was tightened so a client-role account can never gain team access even via a stray assignment row. Team roles landing on `/portal` bounce to `/`; client roles landing anywhere else bounce to `/portal`.
- **AI assistant (§20)** — an **Assistant** tab per client: three preset prompts (content ideas, meeting prep, strategy gaps) plus free text, streaming answers. The server route (`app/api/assistant`) builds its grounding context through the **signed-in user's own RLS-scoped Supabase client**, so the model only ever sees what the asking user could see. Claude Opus 5 (`claude-opus-5`) via `@anthropic-ai/sdk`, with the server-side safety-classifier fallback enabled. Requires `ANTHROPIC_API_KEY` in the environment; without it the tab shows a friendly "not switched on yet" notice and the route returns 503 — nothing breaks.
- **Content production workflow (Duane's spec, Aug 2026)** — the pipeline is now Idea → Approved for production → In production → Ready for approval → (Changes requested) → Ready to schedule → Scheduled → Published, with two distinct approvals (idea vs final content). One **master content record** carries strategy and production status; **`content_outputs`** holds one row per platform version (own caption, CTA, hashtags, links, schedule, status, live URL, reach/engagement/views). Approving an idea opens a confirm dialog (owner, dates, platforms, requirements, approver) and auto-creates a linked production **Action** with the standard checklist; requesting changes — from the team or the client portal — reopens that Action via a SECURITY DEFINER trigger. A **Ready to Schedule queue** sits at the top of the Content tab; scheduling stamps date/time and the post appears on the **Calendar** (which gained month + list views, a client filter, times on chips, and scheduled/published/overdue distinctions). Publishing captures live URL + actual date/time; when every platform version is published the master rolls up to Published and the Action closes. The Dashboard shows the pipeline counts (in production, awaiting approval, changes requested, ready to schedule, scheduled/published this week, overdue). Portal clients approve final content from their Content page. Metrics and performance reports now aggregate from published outputs. Media upload per output and drag-to-calendar are the next slices.

- **PBOS Sales and Client Sales, separated (Duane's spec, Sep 2026)** — one Sales area had been doing two unrelated jobs: Aligned Media selling PBOS, and a client selling their own services. They are now two systems that share no table, no target and no screen.

  **PBOS Sales** (`/sales`, nav "PBOS Sales") is the operator dashboard: monthly target, PBOS revenue this month, MRR, new business won, open pipeline and weighted forecast, closes in 30 days, open leads, conversion rate, overdue sales actions, plus clients-by-tier and deals-by-stage. `pbos_leads` → `pbos_opportunities` → (Won) → `clients` + `pbos_engagements`, with `pbos_lead_actions` for next steps and `pbos_tiers` holding the four tiers. Business-level RLS via a new `is_operator()` (admins and members only — never contractors, never portal clients).

  **The four tiers** — Self-Serve, Guided, Managed, Partner — are a seeded table, not a hard-coded list, so pricing can change without rebuilding anything. Each deal and each engagement stores its own agreed setup fee, monthly fee, extras (with their recurring value), contract start and term; `expected_mrr` is a generated column so the recurring figure can never drift from the parts it's made of. Tier defaults only prefill a new deal — changing them never rewrites a deal already done. Every money column is nullable: the structure ships before the pricing does.

  **A prospect is not a client until the deal is won.** Leads have no client record, no delivery tabs and no portal; a database check constraint makes "won" and "has a client record" the same fact. Marking a deal **Won** creates the client automatically — name, company, contact details, tier as the package, expected MRR as the retainer, LinkedIn carried into the Social tab — with status **`onboarding`**, a new `client_status` value. Won starts onboarding; it doesn't do it.

  **Client Sales** (`/clients/[id]/sales`) is now genuinely theirs: their commercial objective, their own monthly revenue target, progress against it from `commercial_outcomes`, their latest commercial snapshot, and the eleven strategy fields. The dashboard KPI that used to add up `commercial_outcomes` — the revenue *clients* generate — and show it against Duane's PBOS target now reports PBOS's own revenue instead.

  Removed: `sales_opportunities` (it hung deals off `clients`, which is what forced every prospect to be faked as a client) and `actions.sales_opportunity_id`. Both were empty on the live project.

  Fixed along the way: the live `create_client_defaults()` had drifted from migration 0008 and stopped inserting the `sales_strategy` row, so every client created from here on would have 404'd on their own Sales tab — restored in 0049 with a backfill.

## Deliberately left out

No "connect LinkedIn" button anywhere; metrics are explained in the UI as hand-entered, matching both briefs' explicit note that LinkedIn doesn't expose personal-profile analytics to third parties. No role-management UI (SQL editor, see Setup). No automated test suite yet (see "What's still unverified"). §26–§29's remaining Phase Two/Three ideas (integrations, notifications, template libraries, white-labelling) stay future work.
