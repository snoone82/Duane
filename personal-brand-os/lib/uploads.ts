...
export function checkUploadSize(file: File): string | null {
  ...
  return `That file is ${formatBytes(file.size)} — uploads are capped at 50 MB. Compress it (a short export or lower bitrate usually does it), or host the full-size version elsewhere and paste the link into the destination field.`;
}
